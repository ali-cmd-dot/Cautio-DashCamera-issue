...
import { getDangerColor, getDangerLabel, getDangerBg, getDangerBorder, DANGER_LEVELS } from "@/lib/colorUtils";
import { findCityCoords } from "@/lib/cityCoords";

// Map only client-side (no SSR)
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => ({
    total:    data.length,
    critical: data.filter((r) => parseInt(r["Days Pending"]) > 30).length,
    high:     data.filter((r) => { const d = parseInt(r["Days Pending"]); return d > 15 && d <= 30; }).length,
  ...
  const sortedCities = useMemo(() => {
    ...
    if (sidebarFilter !== "all") {
      entries = entries.filter(([, issues]) => {
        const mx = Math.max(...issues.map((i) => parseInt(i["Days Pending"]) || 0));
    ...
    return entries.sort(([, a], [, b]) => {
      const ma = Math.max(...a.map((i) => parseInt(i["Days Pending"]) || 0));
      const mb = Math.max(...b.map((i) => parseInt(i["Days Pending"]) || 0));
      return mb - ma;
    });
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: "#060b12", color: "#e2e8f0" }}>
      ...
      <header className="flex items-center justify-between px-5 py-3 border-b shrink-0"
        style={{ borderColor: "#0f1d2e", background: "rgba(6,11,18,0.98)", zIndex: 30 }}>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center text-base"
              style={{ background: "linear-gradient(135deg,#1e3a5f,#0f1d2e)", color: "#f97316" }}>
              ⬡
            </div>
            <div>
        ...
        <div className="mx-4 mt-3 px-4 py-3 rounded-xl text-sm flex items-start gap-3 shrink-0"
          ...
          <div>
            ...
            <div className="text-xs mt-1 opacity-50">
              Sheet ko "Anyone with link → Viewer" set karo, phir refresh karo.
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {/* Legend */}
          <div className="absolute bottom-4 left-4 z-20 rounded-xl px-4 py-3"
            style={{ background: "rgba(10,16,28,0.92)", border: "1px solid #0f2440", backdropFilter: "blur(12px)" }}>
            ...
            {DANGER_LEVELS.map(({ label, sublabel, color }) => (
              <div key={label} className="flex items-center gap-2.5 mb-1.5 last:mb-0">
                <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: color, boxShadow: `0 0 5px ${color}80` }} />
          ...
          {loading && data.length === 0 && (
            <div className="absolute inset-0 z-30 flex items-center justify-center" style={{ background: "rgba(6,11,18,0.85)" }}>
              <div className="text-center">
          ...
          <div className="flex-1 overflow-y-auto">
            {sortedCities.length === 0 && !loading && (
              <div className="px-4 py-8 text-center text-xs" style={{ color: "#2d4a6a" }}>
                {data.length === 0 ? "No data loaded. Check sheet access." : "No cities match filter."}
              </div>
            )}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!hasMapped && (
                        <span className="text-[9px] px-1 py-0.5 rounded"
                          style={{ background: "#0f1d2e", color: "#2d4a6a" }} title="Not shown on map">?</span>
                      )}
                    </div>
